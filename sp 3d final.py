...
class Probe(Planet):
    def setupStages(self):
        ...
        self.phi0 = 0 
        self.th0 = 0
        self.stage1 = 1 #stage1 thrust time counter for thrust acceleration
        self.stage2 = 0
        self.ts1 = 47005 #stage1 start time
        self.s1dur = 80 #stage1 thrust duration
        self.ts2 = self.ts1 + self.s1dur+1 #stage2 start time
        self.s2dur = 195 #stage2 thrust duration
        self.ts2f = self.ts2+self.s2dur+1 #changed dt for more controled burntime/flight path
        self.taa = 189716 #time to start thrusters for astroid approach
        self.taadur = 25 #time astroid approach duration
        self.taa2 = 194689 #second stage of astroid approach (changing thrust angles)
        self.taadur2 = 335 #second stage duration
        self.tdropao = 200850 #time to drop out of asteroid orbit
        self.tal = 201480 #time to start launding thrusters
        self.taldur = 16 #landing duration
        self.tadep = 408250 #time asteroid launch
        self.tadepdur = 667 #thrust depature duration
        self.tea = 509888 #time start thrusters to enter earth orbit
        self.teadur = 338 
        self.tap = 520003 #time for second stage of thruster to enter earth orbit
        self.tapdur = 495
...
"""(x,y,z,vx,vy,vz)"""
pj = 4.944798694191398 #perihelion Jupiter
...
"""graph orbits"""
#graph(title, bodies, startTime=, endTime=, Tilt=, Rotation=)
graph("A",array([sun,jupiter,earth,ida,probe]))
graph("B",array([sun,jupiter,earth,ida,probe]),tilt=0)
graph("C",array([sun,jupiter,earth,ida,probe]),tilt=90)
